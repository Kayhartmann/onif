...
### Changed
- Replace internal Mosquitto broker with HA's built-in MQTT broker (`services: mqtt:need`)
- MQTT credentials now provided by the HA supervisor; no separate Mosquitto add-on required
- Dashboard MQTT status chip renamed from "Mosquitto" to "MQTT Broker"
- Neolink and dashboard connect to HA MQTT broker instead of `127.0.0.1:1883`

### Removed
- Internal Mosquitto service (no longer needed)
